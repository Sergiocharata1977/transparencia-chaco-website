...
const actualizarCalleSchema = z.object({
  ...
  cuadras100: z.number().min(0).optional(),
  cuadras50: z.number().min(0).optional(),
  cuadrasAsfaltadas: z.number().min(0).optional(),
  cuadrasTierra: z.number().min(0).optional(),
  ...
  try {
    ...
    if (shouldAppendHistorial(data)) {
      updatePayload.historial = FieldValue.arrayUnion(cleanUndefined({
        ...
        longitudMetros: data.longitudMetros ?? current.longitudMetros ?? 0,
        cuadrasAsfaltadas: data.cuadrasAsfaltadas ?? current.cuadrasAsfaltadas ?? 0,
        cuadrasTierra: data.cuadrasTierra ?? current.cuadrasTierra ?? 0,
